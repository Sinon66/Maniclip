...
parser.add_argument('--state_mod_warmup_steps', default=0, type=int,
                    help='disable applying state gate for the first N global steps (default: 0)')
parser.add_argument('--state_mod_floor', default=0.0, type=float,
                    help='minimum gate value in [0,1). Effective gate = floor + (1-floor)*sigmoid(...) (default: 0.0)')
parser.add_argument('--state_mod_reg_weight', default=0.0, type=float,
                    help='regularize gate to stay open: adds reg_weight * mean(1 - s) (default: 0.0)')
...
def main_worker(gpu, args):
    ...
    if hasattr(model, 'state_modulator'):
        model.state_modulator.floor = args.state_mod_floor
...
def train(train_loader, model, discriminator, writter, generator, clip_loss, optimizer, epoch, args, iteration_num=0):
    ...
    for i, (clip_text, sampled_text, labels, exist_mask, length) in enumerate(train_loader):
        ...
        for mb_start in range(0, batch_size, microbatch_size):
            ...
            # Only apply subgroup-invariance / keep constraints when the protected attribute
            # is NOT explicitly specified by the instruction text (exist_mask == 0).
            protect_in_text = None
            protect_free_mask = None
            if (args.use_adv or args.use_counterfactual):
                protect_in_text = exist_mask_mb[:, args.protect_attr_idx]  # [B], float in {0,1}
                protect_free_mask = (protect_in_text < 0.5)
            ...
            clip_text_keep = None
            keep_idx = None
            if args.use_counterfactual and cf_enabled_this_step and protect_free_mask is not None:
                # Only build / run keep branch on samples where the protected attribute is NOT in the text.
                if torch.any(protect_free_mask):
                    keep_idx = protect_free_mask.nonzero(as_tuple=False).squeeze(1)
                    ...
                    sampled_text_keep = [sampled_text_base[j] for j in keep_idx.tolist()]
                    keep_tokens = []
                    for text in sampled_text_keep:
                        if cache_enabled:
            ...
            g_used = None
            if args.use_state_mod and (args.state_mod_warmup_steps <= 0 or global_step >= args.state_mod_warmup_steps):
                g_used = g
            ...
            offset_keep = None
            delta_keep_flat = None
            gen_im_keep = None
            if keep_idx is not None:
                writter.add_scalar('Train/keep_batch_frac', float(keep_idx.numel()) / float(mb_count), global_step)
                styles_keep = styles[keep_idx]
                g_keep = None
                if args.use_state_mod and g is not None:
                    g_keep = g[keep_idx]
                g_keep_used = None
                if args.use_state_mod and g_keep is not None and (args.state_mod_warmup_steps <= 0 or global_step >= args.state_mod_warmup_steps):
                    g_keep_used = g_keep
                offset_keep = model(styles_keep, clip_text_keep, g_keep_used)
                delta_keep_flat = offset_keep.reshape(offset_keep.size(0), -1)
                new_styles_keep = styles_keep.unsqueeze(1).repeat(1, 14, 1) + offset_keep
            ...
            with torch.no_grad():
                offset_l2 = delta_base_flat.pow(2).mean().sqrt()
                img_l1 = (gen_im_base - input_im).abs().mean()
            writter.add_scalar('Train/offset_l2', offset_l2.item(), global_step)
            writter.add_scalar('Train/img_l1', img_l1.item(), global_step)
            if gen_im_keep is not None and keep_idx is not None:
                with torch.no_grad():
            ...
            if args.loss_clip_weight:
                ...
                if gen_im_keep is not None and clip_text_keep is not None:
                    # keep branch only covers a subset; scale its contribution by subset fraction
                    loss_clip_keep = clip_loss(gen_im_keep, clip_text_keep)
                    loss_clip_keep = torch.diag(loss_clip_keep).mean()
                    frac = float(gen_im_keep.size(0)) / float(gen_im_base.size(0))
                    loss_clip = loss_clip + loss_clip_keep * frac
                    writter.add_scalar('Train/CLIP_keep', loss_clip_keep.item(), global_step)
                    writter.add_scalar('Train/CLIP_keep_frac', frac, global_step)
            ...
            if args.use_adv and discriminator is not None and g is not None and protect_free_mask is not None:
                ...
                # Only adversarially regularize samples where protected attribute is NOT required by the text.
                adv_idx = protect_free_mask.nonzero(as_tuple=False).squeeze(1)
                if adv_idx.numel() > 0:
                    delta_adv_base = delta_base_flat[adv_idx]
                    g_adv = g[adv_idx]

                    logits_adv_base = discriminator(grad_reverse(delta_adv_base, args.grl_lambda))
                    loss_adv_base = F.cross_entropy(logits_adv_base, g_adv.long(), reduction='mean')

                    loss_adv = loss_adv_base
                    logits_for_acc = logits_adv_base
                    g_for_acc = g_adv

                    if gen_im_keep is not None and delta_keep_flat is not None and args.adv_apply_to == 'both':
                        # keep branch already only runs on adv_idx samples (keep_idx == adv_idx)
                        logits_adv_keep = discriminator(grad_reverse(delta_keep_flat, args.grl_lambda))
                        loss_adv_keep = F.cross_entropy(logits_adv_keep, g_adv.long(), reduction='mean')
                        loss_adv = 0.5 * (loss_adv_base + loss_adv_keep)
                        logits_for_acc = torch.cat([logits_adv_base, logits_adv_keep], dim=0)
                        g_for_acc = torch.cat([g_adv, g_adv], dim=0)

                    loss = loss + loss_adv * adv_weight
                    adv_losses.update(loss_adv.item(), int(delta_adv_base.size(0)))
                    ...
                    # No eligible samples in this microbatch
                    adv_losses.update(0.0, 1)
                    writter.add_scalar('Train/loss_adv', adv_losses.avg, global_step)
                    writter.add_scalar('Train/D_acc', 0.0, global_step)
            ...
            if args.use_counterfactual and g is not None and cf_enabled_this_step and gen_im_keep is not None and keep_idx is not None:
                ...
                keep_losses.update(loss_keep.item(), int(gen_im_keep.size(0)))
                writter.add_scalar('Train/L_keep', keep_losses.avg, global_step)
                with torch.no_grad():
                    keep_preds = logits_keep.argmax(-1)
                    protect_flip_rate = (keep_preds[:, args.protect_attr_idx] != g_keep_lbl).float().mean().item()
                protect_flip_rates.update(protect_flip_rate, int(gen_im_keep.size(0)))
                writter.add_scalar('Train/protect_flip_rate', protect_flip_rates.avg, global_step)
            elif args.use_counterfactual and g is not None and cf_enabled_this_step:
                # Counterfactual enabled, but no eligible samples (protected attribute mentioned in text for all samples).
                keep_losses.update(0.0, 1)
                writter.add_scalar('Train/L_keep', keep_losses.avg, global_step)
                protect_flip_rates.update(0.0, 1)
                writter.add_scalar('Train/protect_flip_rate', protect_flip_rates.avg, global_step)
...
class StateModulator(nn.Module):
    ...
    def forward(self, text_embedding: Tensor, g: Tensor) -> Tensor:
        ...
        floor = float(getattr(self, 'floor', 0.0))
        if floor > 0.0:
            s = floor + (1.0 - floor) * s
